...
def normalize_json_string(s: _S) -> _S:
    """
    Normalize s from to string (or list of string) or JSON String (or
    list JSON String) to simply string (or list of string).
    For example, this would map a jwt token "ey...Ui5" to ey...Ui5
    (note the missing quote symbol).
    """
    if isinstance(s, str):
        return s.strip('"')
...
def _check_http_post_headers(context: satosa.context.Context) -> None:
    ...
    if (http_method := context.request_method.upper()) != "POST":
        raise AuthRespParsingException(f"HTTP method [{http_method}] not supported", f"HTTP method [{http_method}] not supported")
    ...
    if context.http_headers:
        ...
        if "application/x-www-form-urlencoded" not in content_type:
            raise AuthRespParsingException(f"HTTP content type [{content_type}] not supported", f"HTTP content type [{content_type}] not supported")
    ...
    def parse_and_validate(self, context: satosa.context.Context) -> AuthorizeResponsePayload:
        ...
        try:
            ...
            if (vp_token := resp_data.get("vp_token", None)):
                # vp_token should be a Json string but caller might not be compliant and use string instead: be flexible
                vp_token = normalize_json_string(vp_token)
                d["vp_token"] = vp_token
